const validator = {
  isValid: function (numeroTarjeta) {
    ...
    for (let i = 0; i < cardSplit.length; i++) {
      //inicie desde la posicion 0 (par) y dependiendo la longitud inicie su ciclo
      if (i % 2 === 1) {
        ...
        if (n >= 10) {
          ...
          cardPair = cardPair + parseInt(n); //devuelve un entero
  ...
  maskify: function (numeroTarjeta) {
    ...
    for (let j = 0; j < nLength; j++) {     //j: contador      
